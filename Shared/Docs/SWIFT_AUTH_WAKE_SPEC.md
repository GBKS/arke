...
2. **Unregister a wake for a different mailbox.** If `mailbox_id` in the payload doesn't match the current wallet's mailbox (wallet was replaced on this device), the wake names an orphaned registration nothing can renew — APNs never reports the token invalid while the app is installed, so the relay would keep a dead worker and daily wakes alive forever. Send `DELETE /v1/register` for the payload's mailbox id + current device token (idempotent; `removed: 0` is success), touching none of the current wallet's registration state, and complete with `.noData` (`.failed` if the request errors, so the next wake retries). *Implemented 2026-09-21 via `RelayRegistrationService.unregisterStaleMailbox`; the mint/register/no-token cases were implemented 2026-09-17.*
