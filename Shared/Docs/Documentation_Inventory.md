...
> **Note on moving files:** Docs live under `Shared/`, which is mostly auto-synced across targets. Markdown files aren't compiled, but after moving/deleting files verify Xcode doesn't show red missing-file references. **Keep basenames unique across all of `Docs/`** — the ArkeDesktop resource copy flattens paths, so two files with the same name in different folders break the macOS build ("Multiple commands produce"), even inside `Archive/`.
>
...
- [x] **Step 15:** ✅ 2026-07-09 — Archived `PASSKEY_INTEGRATION_PLAN.md` + `_REVIEW.md` to `Archive/Implementations/` with "not pursued" notes. Decision (Christoph): passkey integration doesn't bring enough benefit; no revival planned. Updated the 1 inbound reference (LINKED_DEVICES_AND_VTXO_SYNC_ANALYSIS related-docs list).
...
**Completed work, archive candidates:** FIX_DATABASE_ERROR_AFTER_DELETION, BEFORE_AFTER_COMPARISON, NETWORK_MISMATCH_UX_CHANGES. *(Device registry/migration docs archived 2026-07-08 — Steps 2 & 4; read-only mode + manual primary assignment archived 2026-07-09 — Steps 11 & 12; LNURL-pay plan archived 2026-07-09 — Step 13; passkey plan + review archived unimplemented 2026-07-09 — Step 15.)*
...
**Note:** Root-level CPFP docs archived to `Archive/Fixes/` (Step 22, 2026-07-09). ⚠️ Step 22's code check found the custom BDK onchain-wallet path (`BDKOnchainWallet`/`BDKCpfpHelper`) is dead code since the switch to Bark's built-in wallet — the *other* BDK/ docs describe that same custom integration and likely need the same verify-then-archive treatment (candidate for a future step; `BDKTransactionReader` is still live for history reading).

---

### Contacts/ (2 files) ✅ RECREATED 2026-07-09 (Step 20)

- `Default_Contact.md`, `Contact_Address_Deletion.md` — living docs moved in from root (Step 20). The 5 completed-migration docs went to `Archive/Implementations/Contacts/` on 2026-07-08 (Step 9).
